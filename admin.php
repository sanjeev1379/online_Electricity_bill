...
if(isset($_POST['admin'])){
  ...
  if(!empty($username)&&!empty($password)){
    $result="select * from admin_login where username='$username'";
    ...
    echo "Error in query.. <br />";
      ...
      if(@($username==$username_database)&&($password==$password_database)){
        //require 'profile.php';
        header("Location:registation.php?q=".$username_database);
      } else {
...
<html lang="en">
<head>
  <title>Electrity Bill | Admin</title>
  <meta charset="utf-8" />
  ...
  <script type="text/javascript">
    ...
    <div class="container" style="border-bottom:solid 1px white;" id="bg_1">
      ...
      <div class="modal-footer">
        <input type="submit" name="admin" id="bt" class="btn btn-primary btn-block" value="Login"  style="width:64%; margin-left:18%;"/>
      </div>
      </form>
    </div></center>
