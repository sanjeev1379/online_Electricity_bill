...
if(isset($_POST['register'])){
  ...
  if(!empty($username)&&!empty($password)&&!empty($address)&&!empty($email)&&!empty($mobile_no)){
    if(@($password_hash==$password_again_hash)){
      ...
      if($query_pro){
        echo "<script>alert('Thanku! you are successfully Registerd! Please Login')</script>";
    ...
    echo  "<script>window.open('user_registation.php','_self')</script>";
  }
}
    ...
    <div class="container">
      <div class="navbar-collapse" id="mainNavBar">
        <ul class="nav navbar-nav navbar-right" style="margin-right:5%;">
          <li><a id="left" href="user.php">Login</a></li>
